<!DOCTYPE html> <!--A program segítségével jön létre az összetevők tábla-->
<html>
    <?php
        session_start();
    ...
    <body>
        ...
        $hozzavalok_listaja=$link->query("SELECT nev From Hozzavalok order by hozzavalo_id");
        $index=0;
        ...
        <form action="recept_megirasa.php?tema=<?php echo $tema?>&recept_id=<?php echo $recept_id?>" method="post">
            <br/> <br/>
        ...
        if(isset($_POST['kesz']))
        {
            $hozzavalok_listaja=$link->query("SELECT nev, hozzavalo_id From Hozzavalok order by hozzavalo_id");
            $legalabb_egy=0;
            $szamlalo=1;
            $index++;
            ...
            if($legalabb_egy==1)
            {
                $atiranyitas= "sikeres_recept_iras.php?tema=$tema"; //csak így működött
                echo "<script>window.location.href = '{$atiranyitas}';</script>";
                exit();
            }
            else echo('<span class="btn btn-<?php echo $tema?>" style="font-size:15px;font-weight: bold;font-style: italic; background-color: red; color:burlywood;margin: 0 auto">Legalbb egy összetevőt kötelező megadni!</span>'); 
        }
        ?>    
        </div>
    </body>
